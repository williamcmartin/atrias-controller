...
GRFy_desired_raibert = cont.data(:,33) .* contact;
GRF_angle_desired_raibert = cont.data(:,34)*180/pi;
plot_fscope('Desired GRFs from Inverse Dynamics - Raibert',{t,t}, {[GRFx_desired_raibert] [GRFy_desired_raibert]}, ...
    {{'Contact','Desired GRFx'},{'Contact','Desired GRFy'}},...
    {'Desired GRFx','Desired GRFy'}, {'Time (sec)','Time (sec)'}, {'Force (N)','Force (N)'}, {[-1000 1000],[0,4000]}, 2, contact_all);
...
spring_deflection_raibert_measured = cont.data(:,38);
plot(t,spring_deflection_raibert_measured);
subplot(212);  hold on;
...
virtual_spring_force_raibert = k_virtual_raibert_measured.*spring_deflection_raibert_measured;
plot(t,virtual_spring_force_raibert);
