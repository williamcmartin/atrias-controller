...
plot_fscope('Leg Position',{t,t}, {[position(:,1:4)*180/pi, kalman_filtered_gear_positions(:,1:2)*180/pi], [position(:,5:8)*180/pi, kalman_filtered_gear_positions(:,3:4)*180/pi]},...
...
plot_fscope('SEA Torques',{t,t}, {[desired_forces(:,1:2) spring_forces(:,1:2)],[desired_forces(:,3:4) spring_forces(:,3:4)]}, ...
    ...
    {'Right SEA Torques','Left SEA Torques'}, {'Time (sec)','Time (sec)'}, {'Torque (Nm)','Torque (Nm)'}, {[-600 600],[-600 600]}, 2, contact_right_left);
...
% leg torque
    ...
    right_leg_torque_desired = torque(:,8)+torque(:,7);
    left_leg_torque_desired = torque(:,11)+torque(:,10);
    
plot_fscope('Leg Torques',{t,t}, {[right_leg_torque_measured right_leg_torque_desired.*contact.*right_left], [left_leg_torque_measured left_leg_torque_desired.*contact.*~right_left]}, ...
    {{'Contact','Measured','Desired'},{'Contact','Measured','Desired'}},...
...
desired_forward_velocity_signal = controller(:,74);
% A_epislon_c_for_x = controller(:,75:77);
% A_epislon_c_no_bias_for_x = controller(:,78:80);
...
% Plot state machine
plot_fscope('State Machine',{t}, {[state_machine, 0.5*right_left]}, ...
...
plot_fscope('Center of Mass',{t,t}, {[x_com_boom, x_com_robot], [dx_com_boom.*contact_nan, dx_com_robot, dx_com_kalman]}, ...
...
GRFx_feedforward_2 = inverse_dynamics_GRFz_target.*x_com_robot./z_com_kalman.*flight_nan;
GRFz_feedforward = @(t) m_total_real*(g + ddz_ref(t));
GRFz_feedforward_1 = inverse_dynamics_GRFz_target.*contact + (k_LQR_vertical*[z_com_kalman-z_reference_during_stance(:,1), dz_com_kalman-z_reference_during_stance(:,2)]')'.*contact;


plot_fscope('Inverse Dynamics',{t,t}, {[inverse_dynamics_GRFx_target.*contact, (right_estimated_grf(:,1)+left_estimated_grf(:,1)).*contact, GRFx_feedforward_1, GRFx_feedforward_2], [inverse_dynamics_GRFz_target.*contact, (right_estimated_grf(:,2)+left_estimated_grf(:,2)).*contact, GRFz_feedforward_1]}, ...
    {{'Target','Estimated','Feedforward','Feedforward'},{'Target','Estimated','Feedforward'}},...
...
plot_fscope('Flight: Foot Position Reference Signals',{t,t,t}, {[desired_foot_position_x(:,1).*contact_nan, x_com_robot, x_touchdown_target.*contact_nan], [desired_foot_position_y(:,1).*contact_nan, y_com_robot, y_touchdown_target.*contact_nan], [desired_foot_position_z(:,1).*contact_nan, z_com_robot.*contact_nan]}, ...
...
plot_fscope('Foot Placement Adaptive Compensation',{t}, {[adaptive_x_foot_compensation.*contact_nan]}, ...
    {{'X'}},...
    {'Foot Correction'}, ...
    {'Time (sec)'}, {'m'}, {'auto'});
...
plot_fscope('Velocity Tracking',{t}, {[0*desired_forward_velocity_signal, dx_com_kalman.*contact_nan]}, ...
    {{'Target','Estimated'}},...
    {'Forward Velocity'}, ...
    {'m/s'}, {'Time (sec)'}, {'auto'});
...
% plot_fscope('Adaptive Model',{t,t}, {[A_epislon_c_for_x], [A_epislon_c_no_bias_for_x]}, ...
