...
%                         right leg length
%
% Note: roll, pitch, & yaw are aligned with the chassis and measured in the world frame
...
function [ com_pelvis, dcom_pelvis, ...
    ...
    com_pelvis_y = R_roll(phi)*R_pitch(theta)*com_pelvis_b;
    com_pelvis_g = R_yaw(psi)*com_pelvis_y; % Wrong order!
    com_pelvis = [com_pelvis_b; com_pelvis_y; com_pelvis_g];

    com_fL_b = com_pelvis_b + pelvis_hipL_b + hipL_fL_b;
    com_fL_y = R_roll(phi)*R_pitch(theta)*com_fL_b;
    com_fL_g = R_yaw(psi)*com_fL_y; % Wrong order!
    com_left_foot = [com_fL_b; com_fL_y; com_fL_g];

    com_fR_b = com_pelvis_b + pelvis_hipR_b + hipR_fR_b;
    com_fR_y = R_roll(phi)*R_pitch(theta)*com_fR_b;
    com_fR_g = R_yaw(psi)*com_fR_y; % Wrong order!
    ...
    dcom_pelvis_y = R_roll(phi)*R_pitch(theta)*dcom_pelvis_b ...
             + R_roll(phi)*dR_pitch(theta,dtheta)*com_pelvis_b ...
             + dR_roll(phi, dphi)*R_pitch(theta)*com_pelvis_b;
    dcom_pelvis_g = R_yaw(psi)*dcom_pelvis_y + dR_yaw(psi,dpsi)*com_pelvis_y; % Wrong order!
    dcom_pelvis = [dcom_pelvis_b; dcom_pelvis_y; dcom_pelvis_g];

    dcom_fL_b = dcom_pelvis_b + dpelvis_hipL_b + dhipL_fL_b;
    dcom_fL_y = R_roll(phi)*R_pitch(theta)*dcom_fL_b...
              + R_roll(phi)*dR_pitch(theta,dtheta)*com_fL_b...
              + dR_roll(phi,dphi)*R_pitch(theta)*com_fL_b;
    dcom_fL_g = R_yaw(psi)*dcom_fL_y + dR_yaw(psi,dpsi)*com_fL_y;  % Wrong order!
    dcom_left_foot = [dcom_fL_b; dcom_fL_y; dcom_fL_g];

    dcom_fR_b = dcom_pelvis_b + dpelvis_hipR_b + dhipR_fR_b;
    dcom_fR_y = R_roll(phi)*R_pitch(theta)*dcom_fR_b ...
              + R_roll(phi)*dR_pitch(theta,dtheta)*com_fR_b...
              + dR_roll(phi,dphi)*R_pitch(theta)*com_fR_b;     
    dcom_fR_g = R_yaw(psi)*dcom_fR_y + dR_yaw(psi,dpsi)*com_fR_y;  % Wrong order!
