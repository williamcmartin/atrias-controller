function [u,y,dy,D_inv]=ATRIAS2D_SS_feedback_control_RoughGround03(q,dq,D,H,G,B,theta_limits,h_alpha,poly_cor,FlagGroundHeightTransition)
%
%% Choices
...
if limit_s
    ...
    if s > 1+delta
        ...
        h_alpha(2,end)=h_alpha(2,end)+(qT-qTdesired)+min(10*(s-1)*3*pi/180,5*pi/180);
...
if or(FlagGroundHeightTransition == 1,FlagGroundHeightTransition == 2) % damping step following impact
    %u = u - 50*dq([2 3 6 7]);
    
    [u,y,dy] = PD(q,dq,D,H,G,B,theta_limits,h_alpha,poly_cor,FlagGroundHeightTransition);    

elseif and(FlagGroundHeightTransition == 2, s<0.5) % remove jerkiness on transition
    uBound=4*ones(4,1);
end
